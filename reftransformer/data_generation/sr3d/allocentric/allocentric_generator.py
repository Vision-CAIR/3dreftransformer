...
class AllocentricGenerator(ReferenceGenerator):
    def __init__(self, verbose=True):
        ...
        with open('sr3d/allocentric/hp.yml') as fin:
            self.hp = yaml.full_load(fin)

    def has_allocentric_relations(self, target, anchor):
        ...
        # Ignore references where an anchor intersects with a target object
        ...
        # Get the oriented section that the target occupy
        ...
        for sec, in_sec_targets in oriented_sections.items():
            ...
            assert len(in_sec_targets.keys()) == 1
            ...
            if target_occupancy_ratios[object_id] > positive_occ_thresh:
                return OrientedSections(sec).name
