...
class HorizontalProximityGenerator(ReferenceGenerator):
    def __init__(self, verbose=True):
        ...
        with open('sr3d/horizontal_proximity/hp.yml') as fin:
            self.hp = yaml.full_load(fin)

    @staticmethod
    def has_horizontal_relation(scan, target, anchor):
        same_class_objects = [obj for obj in scan.three_d_objects if
                              obj.instance_label == target.instance_label and obj.object_id != target.object_id]

        if len(same_class_objects) == 0:
            return None

        all_dists = [anchor.distance_from_other_object(target)]

        for target in same_class_objects:
