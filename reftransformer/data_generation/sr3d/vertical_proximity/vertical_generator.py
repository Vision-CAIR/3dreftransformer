...
class VerticalProximityGenerator(ReferenceGenerator):
    def __init__(self, verbose=True):
        ...
        with open(
                'sr3d/vertical_proximity/hp.yml') as fin:
            self.hp = yaml.full_load(fin)

        # Read the necessary dictionary
        with open(
                '../data/language/sr3d/semantics/instances_that_can_support.json') as fin:
            self.instance_can_support = json.load(fin)

        with open(
                '../data/language/sr3d/semantics/instances_that_cannot_be_supported.json') as fin:
            self.instance_cannot_be_supported = json.load(fin)

    def has_vertical_relation(self, target, anchor):
        min_above_below_distance = 0.01
