...
if __name__ == '__main__':
    ...
    if not os.path.isdir('visual_transformer_exp_best'):
        os.mkdir('visual_transformer_exp_best')
    ...
    for i_cfg in res:
        ...
        cfg['log_dir'] = 'visual_transformer_exp_best_{}'.format(counter)
        ...
        commands[
            counter] = 'CUDA_VISIBLE_DEVICES={} python train_reftransformer.py --config visual_transformer_exp_best/{}.json'.format(
            gpu, counter)

        with open('visual_transformer_exp_best/{}.json'.format(counter), 'w') as fout:
    ...
    with open('visual_transformer_exp_best_command_look_up.json.txt', 'w') as fout:
        json.dump(commands, fout, sort_keys=True, indent=4)

    t.to_excel('visual_transformer_exp_best_table.xlsx')