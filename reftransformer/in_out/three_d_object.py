...
class ThreeDObject(object):
    ...
    def sample(self, n_samples, rnd_state, normalized_pc=False):
        ...
        idx = rnd_state.choice(n_points, n_samples, replace=n_points < n_samples)
    ...
    def sample_context_points(self):
        if self.context_points is None:
            # Get the bounding box information of the object
            bbox = self.get_bbox(axis_aligned=True)
            bbox_center = np.asarray([bbox.cx, bbox.cy, bbox.cz])
            bbox_dims = np.asarray([bbox.lx, bbox.ly, bbox.lz])
            bbox_context_dims = (np.max(bbox_dims) * 2) ** 2

            # Get the context points
            context_points_dist = np.sqrt((self.scan.pc - bbox_center) ** 2)
            context_points = self.scan.pc[context_points_dist > bbox_context_dims]
            self.context_points = context_points
        return self.context_points
