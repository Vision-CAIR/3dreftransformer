...
"""
import torch
from reftransformer.models import MLP
import torch.nn as nn
from reftransformer.models import LSTMEncoder

from reftransformer.models import load_glove_pretrained_embedding, make_pretrained_embedding
from reftransformer.in_out.vocabulary import Vocabulary

try:
    from reftransformer.models import PointNetPP
except ImportError:
    PointNetPP = None
...
def token_encoder(vocab: Vocabulary,
    ...
    '''
    model = TransformerEncoder(word_embedding_dim, word_embedding)
    '''
    return model
...
class ObjectLangSoftmaxClassifier(nn.Module):
    def __init__(self, dim, var=1.0):
        super().__init__()

        self.dim = dim
        self.var = var
        self.scale_value = (var * ((dim - 2) ** 2 / dim)) ** 0.25

    def normalize(self, data: torch.Tensor, scale_value: float = 1., detach: bool = False, dim=-1) -> torch.Tensor:
        """
        Credits: Ivan Skorokhodov
        From https://github.com/universome/nm-zsl/blob/master/src/utils/training_utils.py#L90
        """
    ...
    def forward(self, input_tuple):
        x, y, x_mask = input_tuple

        # Scale both modalities
        x_normalized = self.normalize(x, scale_value=self.scale_value)

        y_normalized = self.normalize(y, scale_value=self.scale_value)

        if x_mask is None:
            return x_normalized @ y_normalized.T
        else:
            return x_normalized @ y_normalized.T * x_mask + (1.0 - x_mask) * -1000000


def object_lang_clf_softmax(in_dim: int) -> nn.Module:
    """
    After the net processes the language and the geometry in the end (head) for each option (object) it
    applies this clf to create a logit.
    ...
    return ObjectLangSoftmaxClassifier(dim=in_dim, var=1.0)
