...
def make_batch_keys(args, extras=None):
    ...
    batch_keys = ['objects', 'tokens', 'target_pos', 'objects_mask', 'language_mask', 'rel_a', 'rel_b',
                  'rel', 'bboxes', 'poses', 'abs_pos']  # all models use these
...
def batch_preprocess(args, batch: dict) -> dict:
    if not args.neg_loss:
        return batch

    bs = batch['objects'].size(0)

    # TODO Ahmed Ask abou this.. Make it from same batch

    #
    # Negative language to The target object feature
    #
    n_lang_lv = args.neg_lang_level

    # Create the neg lang mask, one when an utterance in the batch matches the criteria
    neg_lan_mask = torch.zeros(bs, bs)  # [b x b]
    ...
    for b_i in range(bs):
        for j in range(bs):
            ...
            l = neg_lan_mask[b_i, j]
            if n_lang_lv >= 1:
                # Different scene and different class type object utterances
                l = (batch['target_class'][b_i] != batch['target_class'][j]) and (
                        batch['scan_id'][b_i] != batch['scan_id'][j])
            if n_lang_lv >= 2:
                # Different/Same scene and different class type object utterances
                l |= batch['target_class'][b_i] != batch['target_class'][j] and (
                        batch['scan_id'][b_i] == batch['scan_id'][j])
            if n_lang_lv >= 3:
                # Different/Same scene and different/same class type object utterances
                l |= (batch['target_pos'][b_i] != batch['target_pos'][j]).item() and (
                        batch['scan_id'][b_i] == batch['scan_id'][j])
                l |= batch['scan_id'][b_i] != batch['scan_id'][j]

            neg_lan_mask[b_i, j] = l

    #
    # Negative object features to The target language (utterance) feature
    #
    max_obj_seq = args.max_distractors + 1
    n_obj_lv = args.neg_obj_level

    # Create the neg obj mask, one when an obj in the batch matches the criteria
    neg_obj_mask = torch.zeros(bs, bs, max_obj_seq)  # [b x b x max_obj_seq_len]

    for b_i in range(bs):
        for j in range(bs):
            # should be different scenes
            if batch['scan_id'][b_i] == batch['scan_id'][j]:
                continue

            l = neg_obj_mask[b_i][j]  # [max_obj_seq_len]

            if n_obj_lv == 1:  # The referential loss
                pass
            elif n_obj_lv == 2:  # objects of different class type in different scenes (take care about the object mask)
                l = (batch['target_class'][b_i] != (batch['class_labels'][j]) * batch['objects_mask'][j])
            elif n_obj_lv == 3:  # all objects in differet scenes
                l = torch.logical_or(batch['objects_mask'][j], l)

            neg_obj_mask[b_i][j] = l

    batch.update({
        'neg_lang_mask': neg_lan_mask,
        'neg_obj_mask': neg_obj_mask
    })

    return batch


def single_epoch_train(model, data_loader, criteria, optimizer, optimizer_transformer, device, pad_idx, args):
    """
    :param model:
    :param data_loader:
    ...
    for _, batch in enumerate(tqdm.tqdm(data_loader, leave=True, position=0)):
        batch = batch_preprocess(args, batch)
        ...
        all_losses = compute_loss(batch, res, criteria, args)
        total_loss = all_losses['total_loss']
        total_loss.backward()
        optimizer.step()
        # optimizer_transformer.step_and_update_lr()
        ...
        if args.lang_cls_alpha > 0 and 'lang_logits' in res:
...
def compute_loss(batch, res, criterion_dict, args):
    ...
    if 'rel_logits' in res and 'rel' in batch:
        total_loss += F.cross_entropy(res['rel_logits'].transpose(2, 1), batch['rel'], ignore_index=7) * 0.5

    # Object pose loss
    if 'poses_logits' in res and 'poses' in batch:
        total_loss += F.cross_entropy(input=res['poses_logits'].transpose(2, 1), target=batch['poses'].long(),
                                      ignore_index=16) * 0.5

    if 'masked_lang_logits' in res:
        masked_lang_cls_loss = F.cross_entropy(input=res['masked_lang_logits'].transpose(2, 1), target=batch['tokens'],
                                               reduction='none')
        total_loss += ((masked_lang_cls_loss * res['lang_inp_mask'].squeeze(-1)).mean()) * 0.5

    if args.neg_loss:
        if args.neg_lang_level >= 1 and 'neg_lang_logits' in res:
            target = torch.zeros(bs).cuda().long()
            target[:] = 0  # always the gt class is the first one
            total_loss += F.cross_entropy(input=res['neg_lang_logits'], target=target) * 0.5
        if args.neg_obj_level >= 1 and 'neg_lang_logits' in res:
            max_obj_seq = args.max_distractors + 1
            target = torch.zeros(bs).cuda().long()
            target[:] = 0  # always the gt class is the first one
            total_loss += F.cross_entropy(input=res['neg_obj_logits'], target=target) * 0.5
...
def evaluate_on_dataset(model, data_loader, criteria, device, pad_idx, args, randomize=False):
    ...
    data_loader.dataset.re_initialize()

    batch_keys = make_batch_keys(args)

    for batch in tqdm.tqdm(data_loader, leave=True, position=0):
        # Move data to gpu
        for k in batch_keys:
            batch[k] = batch[k].to(device)
