...
def get_siamese_features(net, in_features, aggregator=None, independent_dim=1):
    ...
    if isinstance(in_features, tuple):
        n_items = in_features[0].size(independent_dim)
    else:
    ...
    for i in range(n_items):
        if isinstance(in_features, tuple):
            out_features.append(net((in_features[0][i, :], in_features[1][i, :], in_features[2][i, :])))
        else:
