...
from reftransformer.in_out.arguments import parse_arguments
from reftransformer.in_out.neural_net_oriented import load_scan_related_data, load_referential_data
from reftransformer.in_out.neural_net_oriented import compute_auxiliary_data, trim_scans_per_referit3d_data
from reftransformer.in_out.pt_datasets.listening_dataset import make_data_loaders
from reftransformer.models.Optim import ScheduledOptim
from reftransformer.utils import set_gpu_to_zero_position, create_logger, seed_training_code
from reftransformer.utils.tf_visualizer import Visualizer
from reftransformer.models.referit3d_pp_net import instantiate_referit3d_pp
from reftransformer.models.referit3d_net_utils import single_epoch_train, evaluate_on_dataset
from reftransformer.models.utils import load_state_dicts, save_state_dicts
from reftransformer.analysis.deepnet_predictions import analyze_predictions
import resource
...
if __name__ == '__main__':
    ...
    # referit_data = referit_data.sample(1000, random_state=2021)
    ...
    model = instantiate_referit3d_pp(args, vocab, n_classes).to(device)

    #
    # Creating different learning rates for each module
    # params = [
    #     {'params': model.object_encoder.parameters(), 'lr': 0.0005},
    #     {'params': model.object_clf.parameters(), 'lr': 0.0005},
    #     {'params': model.language_encoder.parameters(), 'lr': 0.0005},
    #     {'params': model.language_clf.parameters(), 'lr': 0.0005},
    # ]

    # params = model.parameters()
    optimizer = optim.Adam(model.parameters(), lr=args.init_lr)
    # optimizer_transformer = ScheduledOptim(
    #     optim.Adam(model.transformer.parameters(), betas=(0.9, 0.98), eps=1e-09),
    #     2.0, args.hidden_dim, 8000)
